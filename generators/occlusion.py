...
class OcclusionGenerator(Generator):
    ...
    def prepare_dataset(self, object_path, data_examples, gt_dict, info_dict, class_to_valid_examples):
       ...
       Prepares the Occlusion dataset and converts the data from the Linemod format to the EfficientPose format
        ...
        # _mask_paths = [img_path.replace("rgb", "mask_all") for img_path in image_paths] #this dir is generated from generate_masks.py script in occlusion_labels and needs also to be copied into the subdir
        _mask_paths = [img_path.replace("jpg", "png") for img_path in image_paths]
        mask_paths = [img_path.replace("rgb", "merged_masks") for img_path in _mask_paths] #this dir is generated from generate_masks.py script in occlusion_labels and needs also to be copied into the subdir
