...
class SuspensionGenerator(Generator):
    """
    ...
    def __init__(self,
                 dataset_base_path,
                 object_ids = {"CapA_bottom": 1, "CapA_top": 2, "CapB_bottom": 3, "CapB_top": 4, "ShaftA": 5, "ShaftB": 6, "SpringA": 7, "SpringB": 8}, #dictionary with the names and object ids of the occlusion dataset
                 image_extension = ".jpg",
                 shuffle_dataset = True,
                  symmetric_objects = {"CapA_bottom": 1, "CapB_bottom": 3, "SpringA": 7, "SpringB": 8}, #set with names and indices of symmetric objects
                 **kwargs):
        """
        ...
        self.class_to_name = {0: "CapA_bottom", 1: "CapA_top", 2: "CapB_bottom", 3: "CapB_top", 4: "ShaftA", 5: "ShaftB", 6: "SpringA", 7: "SpringB"}
        ...
        self.name_to_mask_value = {"CapA_bottom": 1, "CapA_top": 2, "CapB_bottom": 3, "CapB_top": 4, "ShaftA": 5, "ShaftB": 6, "SpringA": 7, "SpringB": 8}
    ...
    def parse_valid_example_id(self, path_to_file):
       ...
       Reads the given file containing a valid example id's of an object
        ...
        """
        path_copy = path_to_file
        try:
            with open(path_to_file) as fid:
                lines = [line.strip() for line in fid if line != ""]
                
            # thre = len(lines[-1].split('s')[0])
            # example_id = (lines[-1][:thre])
            example_id = lines
            return example_id
        except:
    ...
    def parse_valid_examples(self, valid_annotations_path, name_to_class_dict):
        ...
        for object_name in name_to_valid_examples.keys():
            ...
            for txt_file in txt_files:
                tmp = txt_file.split('/')
                tmp2 = tmp[-1].split('.')
                valid_object_ids.append(tmp2[0])
            valid_object_ids =  tuple(valid_object_ids)   
    ...
    def prepare_dataset(self, object_path, data_examples, gt_dict, info_dict, class_to_valid_examples):
       ...
       Prepares the Occlusion dataset and converts the data from the Linemod format to the EfficientPose format
        ...
        # import pdb;pdb.set_trace()
        _mask_paths = [img_path.replace("rgb", "merged_masks") for img_path in image_paths] #this dir is generated from generate_masks.py script in occlusion_labels and needs also to be copied into the subdir
        mask_paths = [img_path.replace("jpg", "png") for img_path in _mask_paths]
        ...
        example_ids = [str(filename.split(".")[0]) for filename in all_filenames]
        # pdb.set_trace()
    ...
    def convert_gt(self, gt_list, info_list, mask_paths):
       ...
       Prepares the annotations from the Linemod dataset format into the EfficientPose format
        ...
        for single_gt_list, info, mask_path in zip(gt_list, info_list, mask_paths):
            ...
            # mask_arrray = np.fromfile(mask_path, np.uint8)
            # mask = cv2.imdecode(mask_arrray, cv2.IMREAD_COLOR)
    ...
    def load_image(self, image_index):
        ...
        # image = np.asarray(Image.open(self.image_paths[image_index]))
        image = cv2.imread(self.image_paths[image_index])
        # img_array = np.fromfile(self.image_paths[image_index], np.uint8)
    ...
    def load_mask(self, image_index):
        ...
        """
        return np.asarray(Image.open(self.mask_paths[image_index]))
        # img_array = np.fromfile(self.image_paths[image_index], np.uint8)
        # image = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        # return image
...
if __name__ == "__main__":
    ...
    train_gen = SuspensionGenerator("../../dataset/Suspension_8object/", use_6DoF_augmentation = True, use_colorspace_augmentation = True)
...
#     def __init__(self,
#                  dataset_base_path,
#                  object_ids, #dictionary with the names and object ids of the occlusion dataset
#                  symmetric_objects, #set with names and indices of symmetric objects
#                  image_extension = ".jpg",
#                  mask_extension = ".png",
#                  shuffle_dataset = True,
...
#         #set the class and name dict for mapping each other
#         pre_class_2_name = self.read_json(self.dataset_base_path +"/" + "class2name.json")
#         pre_class_2_namess = {int(key): val for key, val in pre_class_2_name.items()}
#         self.class_to_name = pre_class_2_namess
#         # print("class_to_name {}".format(self.class_to_name))
        
#         self.name_to_class = {val: key for key, val in self.class_to_name.items()}
#         self.object_ids_to_class_labels, self.class_labels_to_object_ids = self.map_object_ids_to_class_labels(self.object_ids, self.name_to_class)
#         ### --- ###
#         self.name_to_mask_value = self.read_json(self.dataset_base_path +"/" + "class.json")
#         ### --- ###
#         # print("obj ids {}".format(object_ids))
#         #check and set the rotation representation and the number of parameters to use
#         self.init_num_rotation_parameters(**kwargs)
...
# ##############################################################################################################################################################################################################################################################
# ##############################################################################################################################################################################################################################################################
#         # print(object_ids_to_class_labels)
#         # print(class_labels_to_object_ids)
# ##############################################################################################################################################################################################################################################################
# ##############################################################################################################################################################################################################################################################
#         return object_ids_to_class_labels, class_labels_to_object_ids
...
#             class_to_model_3d_diameters[class_label] = all_models_dict[object_id]["diameter"]
#             name_to_model_3d_diameters[class_to_name[class_label]] = all_models_dict[object_id]["diameter"]
#             # print(name_to_model_3d_diameters)
#         return class_to_model_3d_diameters, name_to_model_3d_diameters
...
#         example_id = str(lines[-1])
#         # print("example_id : {}".format(example_id)) #### example_id : 139s_cam_angle4
#         ###############################################################################################################################
...
#         for model_name, class_idx in name_to_object_id.items():
#             filename = "obj_{:02}.ply".format(class_idx)
#             # print("match ply and obejct name {} : {}".format(filename, model_name))
#             point_file = os.path.join(all_models_path, filename)
...
#         vertex = model_data['vertex']
#         points_3d = np.stack([vertex[:]['x']*1000, vertex[:]['y']*1000, vertex[:]['z']*1000], axis = -1).astype(np.float32)
#         # points_3d = np.stack([vertex[:]['x'], vertex[:]['y'], vertex[:]['z']], axis = -1).astype(np.float32)
#         ####################################################################################################################################################################
#         ####################################################################################################################################################################
...
#         # if 'face' in model_data:
#         #     tri_idx = model_data['face']['vertex_indices']
...
#         all_filenames = natsorted(all_filenames)
#         image_paths = [os.path.join(all_images_path, filename) for filename in all_filenames]
#         mask_paths = [img_path.replace("rgb", "merged_masks") for img_path in image_paths]
#         mask_paths = [mask_path.replace(self.image_extension, self.mask_extension) for mask_path in mask_paths] #this dir is generated from generate_masks.py script in occlusion_labels and needs also to be copied into the subdir
#         # print(mask_paths)
#         depth_paths = [img_path.replace("rgb", "depth") for img_path in image_paths]
...
#         example_ids = [str(filename.split(".")[0]) for filename in all_filenames]
#         ###############################################################################################################################
#         filtered_gt_lists = [(gt_dict[key], key) for key in example_ids]
#         # print(filtered_gt_lists)
#         #filer out invalid annotations and annotations from objects not included in the occlusion dataset
#         filtered_gts = [[anno for anno in gt_list if anno["obj_id"] in self.object_ids.values() and example_id in class_to_valid_examples[self.object_ids_to_class_labels[anno["obj_id"]]]] for gt_list, example_id in filtered_gt_lists]
...
#             from PIL import Image
#             mask = np.asarray(Image.open(mask_path))
#             # Image.fromarray(((mask==3)*250).astype(np.uint8))
...
#                 ##############################################################################################################################################################################################################################################################
#                 ##############################################################################################################################################################################################################################################################
#                 # print(self.object_ids_to_class_labels[gt["obj_id"]])
#                 # print(self.name_to_mask_value[self.class_to_name[self.object_ids_to_class_labels[gt["obj_id"]]]])
#                 # print(annotations["bboxes"][i, :])
#                 # A=annotations["bboxes"][i, :]
#                 # plt.imshow(mask)
#                 # plt.show()
#                 # ##############################################################################################################################################################################################################################################################
#                 ##############################################################################################################################################################################################################################################################
