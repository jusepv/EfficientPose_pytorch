...
def focal(alpha=0.25, gamma=1.5):
    ...
    def _focal(y_true, y_pred):
        ...
        indices = torch.where(torch.not_equal(anchor_state, -1))
        import pdb;pdb.set_trace()
        labels = torch.gather(labels, 1, indices)
        classification = torch.gather(classification, 1, indices)

        # compute the focal loss
        alpha_factor = torch.ones_like(labels) * alpha
        alpha_factor = torch.where(torch.equal(labels, 1), alpha_factor, 1 - alpha_factor)
        focal_weight = torch.where(torch.equal(labels, 1), 1 - classification, classification)
        focal_weight = alpha_factor * focal_weight ** gamma
        cls_loss = focal_weight * F.binary_cross_entropy(classification, labels, reduction='none')

        # compute the normalizer: the number of positive anchors
        normalizer = torch.where(torch.equal(anchor_state, 1))
        normalizer = torch.max(torch.tensor(1.0, dtype=torch.float32), normalizer.shape[0])
        cls_loss = torch.sum(cls_loss) / normalizer
...
def smooth_l1(sigma=3.0):
    ...
    def _smooth_l1(y_true, y_pred):
        ...
        regression = torch.gather(regression, 1, indices)
        regression_target = torch.gather(regression_target, 1, indices)
        ...
        # compute the normalizer: the number of positive anchors
        normalizer = torch.maximum(torch.tensor(1), indices.shape[1])
...
def transformation_loss(model_3d_points_np, num_rotation_parameter):
    ...
    """
    model_3d_points = torch.tensor(model_3d_points_np)
    ...
    def _transformation_loss(y_true, y_pred):
        ...
        regression_rotation = regression_rotation[indices] * math.pi
        regression_translation = regression_translation[indices]

        regression_target_rotation = regression_target_rotation[indices] * math.pi
        regression_target_translation = regression_target_translation[indices]
        is_symmetric = is_symmetric[indices]
        ...
        selected_model_points = model_3d_points[class_indices]
        ...
        regression_target_translation = regression
        ...
        sym_indices = torch.where(is_symmetric == 1)
        asym_indices = torch.where(is_symmetric != 1)
...
def dot(vector1, vector2, axis=-1, keepdims=True, name=None):
    ...
    """
    return torch.sum(vector1 * vector2, dim=axis, keepdim=keepdims)
...
def rotate(point, axis, angle, name=None):
    ...
    Rotates a vector v ∈ ℝ³ into a vector v' ∈ ℝ³ using the Rodrigues' rotation formula:
    v' = v * cos(theta) + (a x v) * sin(theta) + a * (a dot v) * (1 - cos(theta)).
    Note:
