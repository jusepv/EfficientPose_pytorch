...
from layers_ssl import ClipBoxes, RegressBoxes, FilterDetections, wBiFPNAdd, BatchNormalization, RegressTranslation, CalculateTxTy, GroupNormalization
...
def build_EfficientPose(phi,
    ...
    #build subnets
    box_net, class_net, rotation_net, translation_net, domain_img_net, domain_instance_net = build_subnets(num_classes,
                                                                                    subnet_width,
                                                                                    subnet_depth,
    ...
    """
    image 사용하려면 image_input 변수 사용하면 되고, feature map 사용하려면 fpn_feature_maps 변수를 사용하면 된다.
    """                                                                  
    
    #apply subnets to feature maps
    classification, bbox_regression, rotation, translation, transformation, bboxes, domain_img, domain_instance = apply_subnets_to_feature_maps(box_net,
                                                                                                                   class_net,
                                                                                                                   rotation_net,
                                                                                                                   translation_net,
                                                                                                                   domain_img_net,
                                                                                                                   ...
                                                                                                                   input_size,
                                                                                                                   anchor_parameters)
    feature_map_out = models.Model(inputs = [image_input, camera_parameters_input], outputs= [fpn_feature_maps])
    ...
    efficientpose_train = models.Model(inputs = [image_input, camera_parameters_input], outputs = [classification, bbox_regression, transformation, domain_instance, domain_img], name = 'efficientpose')
    ...
    filtered_detections = FilterDetections(num_rotation_parameters = num_rotation_parameters,
                                           ...
                                           )([bboxes, classification, rotation, translation, domain_img])
    """
    efficientpose_prediction은 validation 용으로만 사용하는거라서 
    학습할때는 efficientpose_train을 사용
    ...
    return efficientpose_train, efficientpose_prediction, all_layers, feature_map_out, filtered_detections
...
def build_subnets(num_classes, subnet_width, subnet_depth, subnet_num_iteration_steps, num_groups_gn, num_rotation_parameters, freeze_bn, num_anchors):
    ...
    Returns:
       The subnetworks
    """
    domain_img_net = Domain_ImgNet()

    domain_instance_net = DomainNet(subnet_width,
                      subnet_depth,
    ...
    return box_net, class_net, rotation_net, translation_net, domain_img_net, domain_instance_net


class Domain_ImgNet(models.Model):
    def __init__(self):
        super(Domain_ImgNet, self).__init__()
        self.depth = 3
        self.level = 64
        self.conv = layers.Conv2D(kernel_size=3, filters=16)
        self.activation_sigmoid = layers.Activation('sigmoid')
        # self.softmax = tf.keras.layers.Softmax()
    def call(self, inputs):
        feature, level = inputs
        size = int(self.level)
        outputs = tf.image.resize(feature, (size, size))
...
class DomainNet(models.Model):
    def __init__(self, width, depth, num_classes = 1, num_anchors = 9, freeze_bn = False, **kwargs):
        super(DomainNet, self).__init__(**kwargs)
        self.width = width
        self.depth = depth
        self.num_classes = num_classes
        self.num_anchors = num_anchors
        ...
        options.update(kernel_initializer)
        self.convs = [layers.SeparableConv2D(filters = self.width, bias_initializer = 'zeros', name = f'{self.name}/domain-{i}', **options) for i in range(self.depth)]
        self.head = layers.SeparableConv2D(filters = self.num_anchors * 1, bias_initializer = PriorProbability(probability = 0.01), name = f'{self.name}/domain-predict', **options)

        self.bns = [[BatchNormalization(freeze = freeze_bn, momentum = MOMENTUM, epsilon = EPSILON, name = f'{self.name}/domain-{i}-bn-{j}') for j in range(3, 8)] for i in range(self.depth)]
        self.activation = layers.Lambda(lambda x: tf.nn.swish(x))
...
def apply_subnets_to_feature_maps(box_net, class_net, rotation_net, translation_net, domain_img_net, domain_instance_net, fpn_feature_maps, image_input, camera_parameters_input, input_size, anchor_parameters):
    """
    ...
    domain_img = [domain_img_net([feature, i]) for i, feature in enumerate(fpn_feature_maps)]
    # import pdb;pdb.set_trace()
    domain_img = layers.Concatenate(axis=1, name='domain_img')(domain_img)
    # import pdb;pdb.set_trace() /
    ...
    domain_instance = [domain_instance_net([feature, i]) for i, feature in enumerate(fpn_feature_maps)]
    domain_instance = layers.Concatenate(axis=1, name='domain_instance')(domain_instance)
    # domain_classification = tf.expand_dims(domain_classification, axis=2,  name='domain')
    # import pdb;pdb.set_trace()
    ...
    # return classification, bbox_regression, rotation, translation, transformation, bboxes, domain
    return classification, bbox_regression, rotation, translation, transformation, bboxes, domain_img, domain_instance
