...
## Added by SJ Choi on 220616 to show obj's top, bottom and side ###
# def draw_bbox_8_2D_color(draw_img, bbox_8_2D, color_top = (0, 0, 255), color_bottom = (255, 0, 0), 
#                    color_side = (255, 255, 255), color_center = (255, 0, 139), thickness = 2):
def draw_bbox_8_2D_color(draw_img, bbox_8_2D, color = (0, 255, 0), thickness = 2):
    """ Draws the 2D projection of a 3D model's cuboid on an image with a given color.
    ...
    ## --- DEFAULT --- ###
    #lower level
    cv2.line(draw_img, bbox[0], bbox[1], color_bottom, thickness)
    cv2.line(draw_img, bbox[1], bbox[2], color_bottom, thickness)
    cv2.line(draw_img, bbox[2], bbox[3], color_bottom, thickness)
    cv2.line(draw_img, bbox[0], bbox[3], color_bottom, thickness)
    #upper level
    cv2.line(draw_img, bbox[4], bbox[5], color_top, thickness)
    cv2.line(draw_img, bbox[5], bbox[6], color_top, thickness)
    cv2.line(draw_img, bbox[6], bbox[7], color_top, thickness)
    cv2.line(draw_img, bbox[4], bbox[7], color_top, thickness)
    #sides
    cv2.line(draw_img, bbox[0], bbox[4], color_side, thickness)
    cv2.line(draw_img, bbox[1], bbox[5], color_side, thickness)
    cv2.line(draw_img, bbox[2], bbox[6], color_side, thickness)
    cv2.line(draw_img, bbox[3], bbox[7], color_side, thickness)
    
    #check if centerpoint is also available to draw
    if len(bbox) == 9:
        #draw centerpoint
        cv2.circle(draw_img, bbox[8], 3, color_center, -1)
...
def draw_annotations(image, annotations, class_to_bbox_3D, camera_matrix, color = (0, 255, 0), label_to_name = None, draw_bbox_2d = False, draw_name = False):
    ...
    for i in range(annotations['bboxes'].shape[0]):
        ...
        ### Added by SJChoi on 220714
        # draw_bbox_8_2D_color(image, points_bbox_2D, color_top = (59, 207, 251), color_bottom = (255, 0, 180), 
        #            color_side = color, color_center = color, thickness = 2)
        draw_bbox_8_2D(image, points_bbox_2D, color = color)
